...
train_dataset = PoseDataset('Dataset Split/Train', class_to_idx)
train_loader = DataLoader(train_dataset, batch_size=32, shuffle=True)

val_dataset = PoseDataset('Dataset Split/Val', class_to_idx)  # NEW: Validation dataset
val_loader = DataLoader(val_dataset, batch_size=32, shuffle=False)

test_dataset = PoseDataset('Dataset Split/Test', class_to_idx)
...
optimiser = torch.optim.AdamW(  # AdamW is better than Adam
    model.parameters(), 
    lr=0.001, 
    weight_decay=0.001,  # Increased regularisation
    betas=(0.9, 0.999)
)
...
for epoch in range(num_epochs):
    ...
    # Writing test metrics to TensorBoard
    writer.add_scalar('Loss/test', test_loss, epoch)
    writer.add_scalar('Accuracy/test', test_accuracy, epoch)
    
    # Saving best model based on VALIDATION accuracy (not test)
    ...
    # Printing epoch summary - ADDED VALIDATION METRICS
    print(f"Epoch {epoch+1}/{num_epochs}: "
          f"\nTrain Loss: {train_loss:.4f}, Train Acc: {train_accuracy:.2f}%, "
          f"\nVal Loss: {val_loss:.4f}, Val Acc: {val_accuracy:.2f}%, "  #Validation metrics
          f"\nTest Loss: {test_loss:.4f}, Test Acc: {test_accuracy:.2f}%")
    print("-------------------------------------------------------------")
    
    # Update learning rate scheduler based on validation loss
    scheduler.step(val_loss)  # Using validation loss for scheduling
