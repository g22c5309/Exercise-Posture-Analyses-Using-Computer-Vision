...
class ST_GCN(nn.Module):
    ...
    3. Using multiple layers to extract motion patterns.
    4. Making a final prediction using a fully connected layer.

    """
    def __init__(self, num_classes, channels=3, num_joints=25):
...
class Spatial_GCN(nn.Module):
    """"
    This class applies a graph convolution over the spatial structure of the pose in a single frame.
    ...
    def __init__(self, in_channels, out_channels, matrix):
        """
        in_channels: Number of input features per joint 
        out_channels: Number of output features after applying the graph conv
        matrix: Adjacency matrix for 17 joints. It's a 25x25 matrix
