...
class PoseDataset(Dataset):
    def __init__(self, npz_folder, class_to_idx, target_frames=32):
        self.data = []
        self.labels = []
...
class_to_idx = {
                'Lunges_correct': 0,
                'Lunges_knee_passes_toe': 1,
                'Lunges_not_low_enough': 2,
                'Plank_arched_back': 3,
                'Plank_correct': 4,
                'Plank_hunch_back': 5,
                'SQUAT_correct': 6,
                'SQUAT_feet_too_wide': 7,
                'SQUAT_front_bent': 8,
                'SQUAT_knees_inward': 9,
                'SQUAT_not_low_enough': 10
                }
...
for epoch in range(num_epochs):
    ...
    scheduler.step(test_accuracy)  # Monitor test accuracy
...
sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', 
            xticklabels=class_to_idx.keys(), 
            yticklabels=class_to_idx.keys())
plt.title(f'Best Model Confusion Matrix (Accuracy: {test_accuracy:.2f}%)')
plt.ylabel('True Label')
plt.xlabel('Predicted Label')
